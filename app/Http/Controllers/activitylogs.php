...
class ActivityLogs extends Controller
{
    ...
    public function saveQuery(Request $request){
        $sql = DB::table('querysave')->get();
        return view('querySaveInterface')->with('sql',$sql);
    }
    public function postSaveQuery(Request $request){

       $querytext = $request->querytext;

       if($querytext=="")
       {
        $dbsave = DB::insert('insert into querysave (dbquery) values(?)', [$querytext]);
       }
       else
       {
        $id=1;
        DB::update('update querysave set dbquery=? where id = ?',[$querytext,$id]);
       }
