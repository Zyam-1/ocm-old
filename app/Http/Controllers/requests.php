...
{   
    ...
    {    
          ...
          App\Http\Controllers\tickets::deptrequest($request->rid);
       ...
       $results = DB::table('results')
                       ->select(
                                ...
                                'results.SignOffByName',
                                'ocmphlebotomy.PhlebotomySampleDateTime'
                            )
                          ...
                          ->join('ocmphlebotomy','ocmphlebotomy.PhlebotomySampleID', 'results.sampleID')
                          ->where('results.request', $request->rid)
                          ->where('results.episode', $request->eid)

                          ->orderBy('results.printpriorty')
                          ->orderBy('results.department')
                        ->groupby('results.Code')
                          ->get();

//   return $results;
                        //   $sampleids = DB::table('ocmrequesttestsdetails')
                        //   ->select('ocmrequesttestsdetails.*','Lists.Text','testdefinitions.shortname as test','testprofiles.name as profileID','ocmphlebotomy.PhlebotomySampleDateTime')
                        //   ->join('Lists','Lists.id', 'ocmrequesttestsdetails.department')
                        //   ->join('testdefinitions','testdefinitions.id', 'ocmrequesttestsdetails.test')
                        //   ->join('testprofiles','testprofiles.id', 'ocmrequesttestsdetails.profileID')
        ...
        // $req=msqli_has_rows($requestInformation);
             ...
             DB::table('Latests')->where('LabNumber',$sampleid_)->delete();
                ...
                if($count5 > 0) {
                  ...
                  foreach($OCMPhlebotomies as $OCMPhlebotomy) {
                        ...
                        [$OCMPhlebotomy->PhlebotomySampleID, $comments['Discipline'], $comments['Comment'], $comments['DateTimeOfRecord'],  date('Y-m-d H:i:s')]);
      ...
      public function downloadReuqest(Request $request)
    {
        ...
        $OCMRequest = DB::table('OCMRequest')
          ...
          $results = DB::table('results')
           ...
           $observations = DB::table('observations')
                          ->whereIn('observations.sampleid',$sampleIDs)
                          ->orderBy('observations.datetime','desc')
                          ->get();                
    ...
    public function index(Request $request, $state='',$patient='',$ward='',$rtype='',$unsigned='')
    {       
         ...
         if ($request->ajax()) {
           ...
           if(!empty($request->from_date))
            ...
            $data = DB::table('OCMRequest')
                                 ...
                                 DB::raw('(SELECT COUNT(*) FROM results 
                                WHERE 
                               
                             results.request = OCMRequest.ReqestID
                                
                            and 
                            (results.SignOffBy=""
                            or
                            results.SignOffBy is NULL
                                  ...
                                  DB::raw('(SELECT COUNT(*) FROM ocmphlebotomy WHERE ocmphlebotomy.PhlebotomyRequestID = OCMRequest.ReqestID) as totalSamples'),
                                    ...
                                    ->leftjoin('users AS B', 'B.id', '=', 'OCMRequest.RequestModifiedBy')
                                     ...
                                     ->when(!empty($request->ward) , function ($query) use($request){
                                         ...
                                         return $query->having('OCMRequest.RequestWardID',$request->ward);
                                     ...
                                     ->when(!empty($request->clinician) , function ($query) use($request){
                                      ...
                                      ->when(!empty($request->to_date) , function ($query) use($request,$to_date){
                                        
                                         return $query->having('OCMRequest.RequestCreatedDateTime', '>=',$request->from_date)->having('OCMRequest.RequestCreatedDateTime', '<=',$to_date);
                                     ...
                                     ->when(!empty($patient) , function ($query) use($request,$patient){
                                       ...
                                       ->when(!empty($request->rtype) , function ($query) use($request){
                                        ...
                                        ->when(!empty($request->state) , function ($query) use($request){
                                         ...
                                         if($request->state == 'SentToTheLab') {

                                            return $query->having('OCMRequest.RequestState','Sent to the lab');
                                         }


                                         if($request->state == 'ReceivedInLab') {

                                            return $query->having('OCMRequest.RequestState','Received In Lab');
                                         }
                     ...
                     $data = DB::table('OCMRequest')
                                    ...
                                    'OCMRequest.bed',
                                     ...
                                     DB::raw('(SELECT count(resulted) FROM results WHERE results.request = OCMRequest.ReqestID and resulted = 1 and SignOffBy is null) as SignOff'),
                                    ...
                                    ->leftjoin('users AS B', 'B.id', '=', 'OCMRequest.RequestModifiedBy')
                                     ...
                                     ->when(!empty($state) , function ($query) use($state){
                                      ...
                                      ->when(!empty($request->state) , function ($query) use($request,$pendingRequests){
                                        ...
                                        ->when(!empty($request->unsigned) , function ($query) use($request){
                                         
                                       
                                        if($request->unsigned == 'ThisWeek') {
                                         ...
                                         return $query->having('OCMRequest.RequestCreatedDateTime', '>=',$weekStartDate)->having('OCMRequest.RequestCreatedDateTime', '<=',$weekEndDate)->having('RequestType','Request');

                                        } 

                                            
                                            if ($request->unsigned == 'Unsigned'){
                                                return $query->having('SignOff','>',0)->having('RequestState','Results Ready');
                                            } 


                                             if ($request->unsigned == 'Pending'){
                                                return $query->having('RequestState','In Progress');
                                            }        
            ...
            return Datatables::of($data)
                    ...
                    ->addColumn('action', function($row){
                           ...
                           $btn .= '
                                <button type="button" id="'.route('requestEpisode').'/'.$row->ReqestID.'/'.$row->RequestEpisodeID.'" data="'.$row->PID.'" title="Add New Request" class="addEpisode btn btn-warning"><i class="fas fa-plus"></i>
                                </button>';
     ...
     if($request->unsigned == 'ThisWeek') {
        ...
        $date2 =  $now->addMonth()->format('Y-m-t'); 

         }

         $CRLists = DB::table('Lists')
         ->where('ListType', '=', 'CR')
         ->get();
...
public function CalculateEfgr($code='',$sex='',$age=''){
...
public static function TestCodes($sample='')
    {
      ...
      $testcodess = DB::table('OCMRequestTestsDetails') 
                          ->where('OCMRequestTestsDetails.sample', $sample)->pluck('profileID');
                          
          return $testcodes0 = DB::table('testprofiles') 
                          ->whereIn('id',$testcodess)->pluck('code')->toArray();
                        
    //             $testcode = DB::table('testdefinitions')
    //             ->where('shortname', $testcodes0[0])->pluck('id')->toArray();
    // // echo $testcode[0];
    // // print_r($testcode);
    // // return 1;
    // // return $testcode;
    ...
    //                 $ProfileID = DB::table('testprofile')
    //                  ->where('TestDefinitionID', $testcode[0])->pluck('ProfileID')->toArray();
    //                       return $ProfileID;
    //                 return $testcodes = DB::table('testprofiles') 
    //                 ->where('id', $ProfileID[0])->select('name')->get()->toArray();
    ...
    public static function profilename($testcode='')
    {
    
          $testcode = DB::table('testdefinitions')->select('id') 
        ->where('shortname', $testcode)->get();


            $ProfileID = DB::table('profiletestmapping')->select('ProfileID') 
                          ->where('TestDefinitionID', $testcode[0]->id)->get();

                         
            return $testcodes = DB::table('testprofiles') 
                          ->where('id', $ProfileID[0]->ProfileID)->select('name')->get();
    }
        ...
        $user = auth()->user();
          ...
          $OCMRequest = DB::table('OCMRequest')
           ...
           $OCMRequestTestsDetails_ = DB::table('OCMRequestTestsDetails')
            ...
            // if (!$OCMRequestTestsDetails->isEmpty()) {
...
// return $request;
        ...
        foreach($patients as $key => $patient) {
          ...
          echo  DB::insert('insert into OCMRequest 
         ...
         echo   DB::insert('insert into OCMRequestDetails (id, RequestID, RequestEpisodeID, TestID, TestDescription, ExecutionDateTime, PatientID) values (?, ?, ?, ?, ?, ?, ?)', 
       ...
       $PhlebotomySampleIDs =  DB::table('OCMPhlebotomy')->where('PhlebotomySampleCollected','!=',null)->where('PhlebotomySampleID',$request->value)->get(); 
        ...
        $RequestClinicianID = $ocmRequest[0]->RequestClinicianID; 
          $RequestPatientID = $ocmRequest[0]->RequestPatientID; 
           ...
           $RequestFasting = $ocmRequest[0]->RequestFasting; 
           ...
           //*
            ...
            die( print_r( sqlsrv_errors(), true));
             ...
             $tsql = "SELECT * FROM ocmRequest where RequestID = '$request->rid' ";
              ...
              $tsql = "INSERT INTO ocmRequest (ID, RequestID, OrderingClinician, RequestState, Chart, PatName, Sex, DoB, Addr0, Addr1, Addr2, Addr3, Ward, Clinician, ClDetails, RooH, AandE, Urgent, GP, Hospital,notes,antibiotics,intendedantibiotics,fasting) 
                             VALUES 
                            (
                                $serverID,
                                $request->rid, 
                 ...
                 $tsql = "INSERT INTO ocmRequestDetails (ID, RequestID, SampleID, SampleDate, TestCode, TestDescription, SampleType, DepartmentID, ProfileID, Programmed, external_) 
                            ...
                            $insertReview = sqlsrv_query($conn_hq, $tsql);
                             ...
                             DB::update("update results  set 
                                ...
                                //         $tsql1 = "update ocmRequestDetails set trans = 1  where  
                                //         TestCode = '".$test."' and
                                //         RequestID = '".$request->rid."' and SampleID = '".$sampleID."'  ";
                                //  sqlsrv_query($conn_hq, $tsql1);
                                //      }
     ...
     public function discardSamplesID(Request $request)
    {
        ...
        $profiles = DB::table('ocmrequesttestsdetails')->where('sampleID', $request->id)->pluck('profileID');
            ...
            DB::table('results')->where('sampleid', $request->id)->delete();
             ...
             $data = DB::table('OCMPhlebotomy')->select('PhlebotomySampleID')
            ->where('PhlebotomyRequestID',$request->rid)
            ->where('PhlebotomySampleCollected','=',null) 
            ->get();
            // $data = DB::table('OCMPhlebotomy')->select('PhlebotomySampleID')
...
public function getBTQuestions(Request $request)
    ...
    public function requestPatient(Request $request)
    {
        ...
        $Visits = 0;
            ...
            // return 1;
        // return $OCMRequest = DB::table('OCMRequest')
        // ->select('OCMRequest.*',
        //          'OCMRequest.id as OID',
        //           'PatientIFs.*', 
      ...
      $quicktestprofiles = DB::table('quicktestprofiles') 
        ...
        $States = DB::table('Lists') 
         ...
         $PatientIFs = DB::table('PatientIFs') 
                         ...
                         ->get();
// return 'SELECT RequestClinicalDetail FROM OCMRequest WHERE OCMRequest.RequestPatientID = "'.$PatientIFs[0]->id.'" order by OCMRequest.id desc limit 1';
            $OCMRequest =DB::select('SELECT RequestClinicalDetail FROM OCMRequest WHERE OCMRequest.RequestPatientID = "'.$PatientIFs[0]->id.'" order by OCMRequest.id desc limit 1');
            if(!$OCMRequest){
				//$requestclinicianDetails
				$OCMRequest=array("0"
				=>array(
				"RequestClinicalDetail"=>""
				)
      ...
      public function requestPatientBT(Request $request)
    {
...
foreach($sampleids as $sampleid) {
 ...
 public function checkBloodGroupInfo(Request $request)
    ...
    $results = DB::table('results')
    ->select('SignOff','SignOffBy','SignOffDateTime','sampleid','Code')->whereIn('Code', $request->tests )->where('request',$request->id)->get()->toArray();
                    ...
                    foreach($results as $result) {
                        ...
                        $SignOff =  1;
                        $SignOffBy =  $from[0]->name;  
                        $SignOffDateTime =$result->SignOffDateTime;
                        $sampleid = $result->sampleid;
                        $Code = $result->Code;

              $tsql11 = "select TargetValue from ocmMapping where SourceValue = '$Code'";
                         $getList1 = sqlsrv_query($conn_hq, $tsql11);
                         $codess = sqlsrv_fetch_array($getList1, SQLSRV_FETCH_ASSOC);
                         $Code2 = $codess['TargetValue'];


    
            $sql311 = "  update Bioresults set SignOff = '$SignOff', SignOffBy = '$SignOffBy', SignOffDateTime = '$SignOffDateTime' where sampleid  = '$sampleid' and Code = '$Code2' ";
            $result1113 = sqlsrv_query($conn_hq, $sql311);
    
    
            $sql312 = "  update Haemresults set SignOff = '$SignOff', SignOffBy = '$SignOffBy', SignOffDateTime = '$SignOffDateTime' where sampleid  = '$sampleid' and Code = '$Code2' ";
            $result1114 = sqlsrv_query($conn_hq, $sql312);


            $sql313 = "  update printvalidlog set SignOff = '$SignOff', SignOffBy = '$SignOffBy', SignOffDateTime = '$SignOffDateTime' where sampleid  = '$sampleid' and Code = '$Code2' ";
            $result1115 = sqlsrv_query($conn_hq, $sql313);
